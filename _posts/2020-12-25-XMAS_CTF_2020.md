...
When i entered the name, It called `check_upper_lower`function. But it only check whether the first character is uppercase or lowercase. It means the name can contain `;` or `|` after first character. So we can use `command injection`.  if i  enter the name as A;/bin/sh, can get the sh.
